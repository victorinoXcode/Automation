...
test.describe("EA Access Management", () => {
  test.describe.configure({mode: "serial"});
  test.setTimeout(120_000);

  test("EA can remove user access", async ({browser}) => {
    qase.id(2203);
    await assertCanRemoveAccess(browser, "ea");
  });

  test("EA can restore user access", async ({browser}) => {
    qase.id(2204);
    await assertCanRestoreAccess(browser, "ea");
  });
});
